...
const BANK_PARTNERS = [
  ...
  { name: "Bank of Baroda", color: "#F04E23", bg: "#FFF1F1", type: "Government Banks", logo: "/bank of baroda.png"},
  ...
  { name: "Union Bank", color: "#005191", bg: "#F0F4FF", type: "Government Banks", logo: "/union.png" },
  { name: "Bank of India", color: "#F04E23", bg: "#FFF1F1", type: "Government Banks", logo: "/bankofindia.png" },
  { name: "UCO BANK", color: "#0067A5", bg: "#F0F7FF", type: "Government Banks", logo: "/uco.jfif" },
  ...
  { name: "ICICI Bank", color: "#F58220", bg: "#FFF7ED", type: "Private Banks", logo: "/icici.png"},
  ...
  { name: "RBL Bank", color: "#005596", bg: "#F0F6FB", type: "Private Banks", logo: "/rbl.png" },
...
function WelcomeScreen({ onComplete }) {
  ...
  return (
    <motion.div
      ...
      className="fixed inset-0 z-[1000] bg-[#0B1F3A] flex flex-col items-center justify-center overflow-hidden"
    >
      {/* Background Video */}
      <video
        autoPlay
      ...
      >
        <source src="/welcome video.mov" type="video/webm" />
      </video>

      {/* 3D Rotating Logo Container */}
      <div className="relative mb-12" style={{ perspective: "1200px" }}>
        <motion.div
          animate={{ rotateY: 360 }}
          transition={{ duration: 3, repeat: Infinity, ease: "linear" }}
          className="w-32 h-32 sm:w-40 sm:h-40 relative flex items-center justify-center"
        >
          {/* Glowing Aura */}
          <div className="absolute inset-0 bg-[#E2C16B]/20 blur-[60px] rounded-full animate-pulse" />

          <div className="bg-white rounded-3xl p-4 shadow-[0_0_50px_rgba(226,193,107,0.3)] border-2 border-[#E2C16B]/50 overflow-hidden">
            <img
              src="/mandanilogo.png"
      ...
      <div className="text-center px-6 z-10">
        <motion.h1
          ...
          className="text-white font-display text-2xl sm:text-3xl md:text-5xl font-bold uppercase mb-3 drop-shadow-[0_0_20px_rgba(255,255,255,0.4)] leading-tight"
        >
          Mandani Associate
        </motion.h1>
        <motion.p
          ...
          className="text-[#E2C16B] text-[9px] sm:text-xs font-black tracking-[0.3em] sm:tracking-[0.4em] uppercase"
        >
      ...
      {/* Decorative Grid */}
      <div className="absolute inset-0 opacity-[0.05] pointer-events-none" style={{ backgroundImage: `radial-gradient(${GOLD_LIGHT} 1px, transparent 1px)`, backgroundSize: '60px 60px' }} />
      <div className="absolute inset-0 bg-gradient-to-b from-[#0B1F3A] via-transparent to-[#0B1F3A] pointer-events-none" />
    </motion.div>
  );
}
...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (meta) {
      meta.setAttribute('content', isDesktopMode ? 'width=1280' : 'width=device-width, initial-scale=1.0');
    }
  }, [isDesktopMode]);
    ...
    <div className="font-body min-h-screen bg-white text-slate-700 " style={{ "--navy": NAVY }}>
      ...
      <div className={`print:hidden fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? "bg-[#0B1F3A]/95 shadow-xl" : ""}`}>
        ...
        <Header scrolled={scrolled} view={view} navTo={navTo} goHomeAndScroll={goHomeAndScroll} startApplication={startApplication}
          ...
          isDesktopMode={isDesktopMode} setIsDesktopMode={setIsDesktopMode} />
      </div>
      ...
      <main>
        ...
        {view === "documents" && <DocumentsView navTo={navTo} goHomeAndScroll={goHomeAndScroll} setSelectedLoanId={setSelectedLoanId} />}
        {view === "loan-details" && <LoanDetailsView loanId={selectedLoanId} navTo={navTo} />}
          ...
          <div className="flex items-center gap-3">
            <div className="relative">
              <div className="w-12 h-12 rounded-full bg-white flex items-center justify-center border-2 border-white/20 overflow-hidden">
                <img src="/mandanilogo.png" className="w-14 h-14 object-contain" alt="Bot Logo" />
              </div>
...
function TrackerView({ navTo }) {
  ...
  const handleSearch = async (e) => {
    ...
    const mockApp = {
      id: searchId,
      name: "User Name",
      status: "Verified", // This would be fetched from Supabase: "Submitted", "Verified", "Sanctioned", or "Disbursed"
      lastUpdate: new Date().toLocaleDateString(),
      note: "Your documents have been verified by our senior consultant. Your file is now being logged with our partner banks for final approval.",
      consultant: "S. Rao"
    };
    ...
    <div className="bg-slate-50 min-h-screen pt-32 pb-20">
      <div className="max-w-4xl mx-auto px-6">
        <div className="text-center mb-12">
          <SectionEyebrow>Live Tracker</SectionEyebrow>
          <h2 className="text-4xl font-display font-bold text-[#0B1F3A] mb-4">Track Your Application</h2>
          <p className="text-slate-500 max-w-lg mx-auto font-medium">Enter your Reference ID or Mobile Number to check real-time progress.</p>
        </div>
        ...
        <form onSubmit={handleSearch} className="max-w-xl mx-auto mb-16 relative">
          <input
            ...
            placeholder="Enter ID (e.g. MA10023456)"
            className="w-full h-16 rounded-2xl border-2 border-slate-200 pl-6 pr-40 text-lg font-bold text-[#0B1F3A] outline-none focus:border-amber-500 shadow-xl transition-all"
          />
          <button
            type="submit"
            disabled={loading}
            className="absolute right-2 top-2 bottom-2 px-8 rounded-xl bg-[#0B1F3A] text-[#E2C16B] font-bold uppercase tracking-widest text-xs hover:bg-slate-800 transition-colors shadow-lg disabled:opacity-50"
          >
        ...
        {result && (
          <motion.div
            ...
            className="bg-white rounded-[2.5rem] border-2 border-slate-100 shadow-2xl p-8 sm:p-12"
          >
            <div className="flex flex-col sm:flex-row justify-between items-start gap-6 mb-12 border-b border-slate-100 pb-12">
              <div>
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Application ID</p>
                <h3 className="text-2xl font-bold text-[#0B1F3A]">{result.id}</h3>
              </div>
              <div className="text-left sm:text-right">
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Assigned Advisor</p>
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-amber-50 flex items-center justify-center text-amber-600">
                    <User size={20} />
                  </div>
                  <p className="font-bold text-[#0B1F3A]">{result.consultant}</p>
                </div>
              </div>
            </div>
            ...
            <div className="relative mb-16 px-4">
              <div className="absolute top-4 left-0 right-0 h-1 bg-slate-100 hidden sm:block" />
              <div className="relative flex flex-col sm:flex-row justify-between gap-8 sm:gap-0">
                {steps.map((s, i) => {
                  const active = i <= getCurrentStepIndex(result.status);
                  const current = i === getCurrentStepIndex(result.status);
                  return (
                    <div key={i} className="flex-1 flex flex-row sm:flex-col items-center gap-4 sm:gap-0 relative z-10">
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center transition-colors border-4 border-white shadow-xl ${
                        active ? 'bg-amber-500 text-white' : 'bg-slate-200 text-slate-400'
                      }`}>
                        {active ? <Check size={14} /> : i + 1}
                      </div>
                      <p className={`mt-0 sm:mt-4 text-[10px] font-black uppercase tracking-widest text-center ${
                        active ? 'text-amber-600' : 'text-slate-400'
                      }`}>
                        {s.label}
                      </p>
                      {current && (
                        <span className="absolute -top-12 px-3 py-1 bg-[#0B1F3A] text-white text-[8px] font-bold rounded-md whitespace-nowrap animate-bounce hidden sm:block">
                          ACTIVE STAGE
                        </span>
                      )}
                    </div>
            ...
            {/* Admin Note Box */}
            <div className="bg-amber-50/50 rounded-3xl p-6 border-2 border-amber-100/50">
              <div className="flex items-center gap-3 mb-4">
                <Shield size={18} className="text-amber-600" />
                <h4 className="text-xs font-black text-amber-800 uppercase tracking-widest">Admin Feedback</h4>
              </div>
              <p className="text-slate-700 text-sm leading-relaxed italic">"{result.note}"</p>
            </div>

            <div className="mt-10 flex flex-wrap gap-4">
               <button className="flex-1 bg-[#0B1F3A] text-white font-bold py-4 rounded-2xl shadow-xl hover:bg-slate-800 transition-all flex items-center justify-center gap-2 group">
                 <MessageCircle size={18} fill="currentColor" /> Chat with Consultant
               </button>
               <button className="flex-1 border-2 border-slate-200 text-slate-700 font-bold py-4 rounded-2xl hover:bg-slate-50 transition-all">
...
function FloatingContactBar() {
  ...
  const actions = [
    { icon: Phone, href: "tel:+919979043073", color: "bg-blue-600", label: "Call Us" },
    {
      icon: MessageCircle,
      href: "https://wa.me/919979043073?text=Hi%20Mandani%20Associate,%20I%20want%20a%20loan.%20Can%20you%20please%20guide%20me?",
      color: "bg-green-500",
      label: "WhatsApp"
    },
  ...
  return (
    <div className="hidden sm:flex items-center justify-end px-6 lg:px-10 py-2 text-xs text-white bg-black/20 backdrop-blur-sm">
      <div className="flex items-center gap-5 opacity-90">
        <span className="inline-flex items-center gap-1.5"><Phone size={12} /> +91 99790 43073, +91 6352243073</span>
...
/* --------------------------------- Header ---------------------------------- */
function Header({ scrolled, view, navTo, goHomeAndScroll, startApplication, mobileNavOpen, setMobileNavOpen, isDesktopMode, setIsDesktopMode }) {
  const links = [
    ...
    <header className={`relative z-50 transition-all duration-300 border-b ${scrolled ? 'bg-transparent border-white/35' : 'bg-white/5 backdrop-blur-lg border-white/40'}`}>
      <div className="px-6 lg:px-10 h-20 flex items-center justify-between max-w-[1440px] mx-auto">
        <button onClick={() => { window.scrollTo({ top: 0, behavior: "smooth" }); navTo("home"); }} className="flex items-center gap-4 group">
          <div className="w-16 h-16 bg-white rounded-2xl flex items-center justify-center shadow-2xl border border-white/20">
            <img
              src="/mandanilogo.png"
              alt="MANDANI"
        ...
        <nav className="hidden lg:flex items-center gap-1">
          {links.slice(0, 5).map(l => (
            <button
              key={l.id}
              onClick={l.action}
              className={`px-4 py-2 text-sm font-bold transition-all relative group ${view === l.id ? 'text-[#E2C16B]' : 'text-white hover:text-[#E2C16B]'}`}
            >
              {l.label}
              <span className={`absolute bottom-0 left-4 right-4 h-0.5 bg-[#E2C16B] transition-transform duration-300 ${view === l.id ? 'scale-x-100' : 'scale-x-0 group-hover:scale-x-100'}`} />
            </button>
          ))}
      ...
      {mobileNavOpen && (
        <div className="lg:hidden fixed inset-x-0 top-20 bottom-0 bg-white z-50 p-6 flex flex-col animate-fade-in overflow-y-auto">
          <div className="space-y-2 flex-grow">
            ...
            {/* Desktop Mode Toggle */}
            <button
              onClick={() => { setIsDesktopMode(!isDesktopMode); setMobileNavOpen(false); }}
              className="w-full text-left p-5 text-lg font-bold rounded-2xl transition-all text-slate-900 border-2 border-slate-100 mt-4 flex items-center justify-between group bg-slate-50/50"
            >
              <div className="flex items-center gap-3">
                <LayoutDashboard size={20} className="text-amber-500" />
                <span>{isDesktopMode ? "Mobile View" : "Desktop View"}</span>
              </div>
              <div className={`w-12 h-6 rounded-full transition-colors relative ${isDesktopMode ? 'bg-amber-500' : 'bg-slate-300'}`}>
                <div className={`absolute top-1 w-4 h-4 bg-white rounded-full shadow-sm transition-all ${isDesktopMode ? 'right-1' : 'left-1'}`} />
              </div>
            </button>
          </div>
    ...
    <div className="absolute inset-0 z-0">
      ...
      <div className="relative z-10 max-w-[1400px] mx-auto px-6 lg:px-10 pt-24 pb-16 grid lg:grid-cols-[1.1fr_0.9fr] gap-12 lg:gap-16 items-center">
        <div className="fade-up text-center lg:text-left">
          ...
          <h1 className="font-display text-white text-4xl sm:text-6xl lg:text-[4.5rem] font-bold leading-[1.05] mb-8 tracking-tight">
            Fast Approvals.<br />
            <span className="text-[#E2C16B] italic">Better Rates.</span>
          </h1>

          <p className="text-white text-lg sm:text-xl leading-relaxed max-w-2xl mx-auto lg:mx-0 mb-6 font-medium opacity-90">
            MANDANI ASSOCIATE connects you with 30+ leading banks to find the perfect loan match. Expert guidance, Our Surat team helps salaried professionals, self-employed individuals and businesses compare suitable loan options from our lending partners..
          </p>

          <div className="flex flex-wrap justify-center lg:justify-start gap-5 mb-2">
            <ScrollReveal direction="left" delay={0.4}>
              <PrimaryButton onClick={() => goHomeAndScroll("loans")} className="!px-10 !py-3 text-lg !rounded-xl shadow-2xl shadow-amber-500/20">
                Explore Loans <ArrowRight size={20} />
              </PrimaryButton>
            </ScrollReveal>
            <ScrollReveal direction="right" delay={0.4}>
              <button onClick={() => navTo("emi")}
                className="inline-flex items-center gap-3 px-10 py-3 rounded-xl font-bold border-2 border-white/20 text-white hover:bg-white/5 transition-all text-lg group backdrop-blur-sm"
              >
        ...
        {/* Floating EMI Card */}
        <div className="fade-up max-w-lg mx-auto lg:mx-0 w-full lg:scale-90 lg:origin-right" style={{ animationDelay: "0.2s" }}>
        <div className="bg-white/50 backdrop-blur-l rounded-[2.5rem] p-7 sm:p-9 shadow-[0_50px_100px_-20px_rgba(0,0,0,0.4)] border border-white/25">
            ...
            <div className="space-y-5 mb-10">
              ...
              <div className="bg-white rounded-2xl p-5 border-2 border-slate-200/60 shadow-xl flex items-start gap-4 transition-all hover:border-amber-500/30">
                ...
                <div>
                  ...
                  <div className="text-base font-bold text-slate-700 leading-tight">+91 99790 43073</div>
                  <div className="text-base font-bold text-slate-700">+91 6352243073</div>
                </div>
              </div>
        ...
        <div className="mt-16 pt-8 border-t border-slate-100">
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-4">
              ...
              <span className="text-sm font-bold text-slate-700">+91 99790 43073 , +91 6352243073</span>
            </div>
            <div className="flex items-center gap-4">
              <div className="w-8 h-8 rounded-lg bg-[#F1F5F9] flex items-center justify-center text-[#3F51B5]">
                <Mail size={16} />
...
/* -------------------------------- DOCUMENTS ----------------------------------- */
function DocumentsView({ navTo, goHomeAndScroll, setSelectedLoanId }) {
  return (
    <div className="bg-white">
      {/* Hero Section */}
      <section className="relative overflow-hidden py-24 lg:py-36 bg-[#0B1F3A]">
                ...
                <ScrollReveal key={loan.id} direction={idx % 2 === 0 ? "left" : "right"} delay={idx * 0.05}>
                  <div className="bg-white/55 backdrop-blur-sm rounded-[2.5rem] border-2 border-slate-200/60 transition-all duration-500 overflow-hidden flex flex-col shadow-lg hover:shadow-2xl hover:border-amber-500/30 group">
                    <div className="p-8 flex-grow">
                      <div className="w-14 h-14 rounded-2xl bg-amber-50 flex items-center justify-center text-amber-600 mb-6 group-hover:bg-amber-600 group-hover:text-white transition-colors duration-500">
                        <loan.icon size={28} />
                      </div>
                      <h3 className="text-xl font-bold text-[#0B1F3A] mb-3">{loan.name}</h3>
                      <p className="text-sm text-slate-500 leading-relaxed">
                        Find the complete list of required documents and specific eligibility criteria for {loan.name.toLowerCase()} in Surat.
                      </p>
                    </div>

                    <button
                      onClick={() => { setSelectedLoanId(loan.id); navTo("loan-details"); }}
                      className="w-full py-5 text-sm font-bold transition-all flex items-center justify-center gap-2 bg-slate-50 text-[#0B1F3A] hover:bg-[#0B1F3A] hover:text-white border-t border-slate-100"
                    >
                      View Requirements & Eligibility <ChevronRight size={18} />
                    </button>
                  </div>
                </ScrollReveal>
              );
          ...
          <ul className="space-y-6">
            <li className="flex items-center gap-4 group">
              ...
              <div className="text-sm font-medium">
                <a href="tel:+919979043073" className="hover:text-amber-400 transition-colors">+91 99790 43073</a> ,
                <a href="tel:+916352243073" className="hover:text-amber-400 transition-colors ml-1">+91 6352243073</a>
              </div>
            </li>
            <li className="flex items-center gap-4 group">
              <div className="w-8 h-8 rounded-lg flex items-center justify-center text-slate-900 shadow-xl shadow-amber-500/10" style={{ backgroundColor: GOLD_LIGHT }}>
...
function LoanDetailsView({ loanId, navTo }) {
  const loan = LOAN_CATEGORIES.find(l => l.id === loanId);
  if (!loan) return (
    <div className="bg-slate-50 min-h-screen py-32 px-6 text-center">
      <div className="max-w-md mx-auto bg-white p-12 rounded-[2.5rem] shadow-xl border border-slate-100">
        ...
        <h2 className="text-2xl font-bold text-slate-900 mb-4">Loan Not Found</h2>
        <p className="text-slate-500 mb-8">The loan category you are looking for does not exist or has been removed.</p>
        <PrimaryButton onClick={() => navTo('home')} full>Back to Homepage</PrimaryButton>
      </div>
    </div>
  );
  ...
  const defaultDocs = [
    ...
    "2 Passport Photos",
    loan.id === 'business' || loan.id === 'personal' ? "Business Proof (GST/MSME)" : "Last 2 Years ITR",
  ];

  return (
    <div className="bg-slate-50 min-h-screen py-12 px-4 sm:px-6 print:py-0 print:px-0 print:bg-white print:min-h-0">
      <div className="max-w-4xl mx-auto bg-white rounded-[2.5rem] shadow-2xl overflow-hidden border border-slate-100 print:shadow-none print:border-none print:rounded-none print:w-full print:max-w-none print:m-0">
        {/* Header */}
        <div className="bg-[#0B1F3A] p-8 md:p-12 text-white relative overflow-hidden print:bg-white print:text-[#0B1F3A] print:border-b-4 print:border-amber-500 print:p-4 print:mb-4">
          <div className="absolute top-0 right-0 w-64 h-64 bg-amber-500/10 rounded-full -translate-y-1/2 translate-x-1/2 blur-3xl print:hidden" />
          <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
            <div className="flex-1">
              <div className="flex items-center gap-4 mb-6">
                <div className="w-14 h-14 bg-white rounded-2xl flex items-center justify-center p-1 shadow-2xl print:shadow-none print:border print:border-slate-200">
                  <img src="/mandanilogo.png" className="w-full h-full object-contain" alt="Logo" />
                </div>
                <div>
                  <h1 className="text-2xl font-bold tracking-tight font-display">MANDANI ASSOCIATE</h1>
                  <p className="text-[10px] font-black tracking-[0.3em] opacity-70 uppercase">Surat's Trusted Loan Experts</p>
                </div>
              </div>
              <h2 className="text-3xl md:text-5xl font-display font-bold mb-2">{loan.name}</h2>
              <div className="flex items-center gap-2 text-amber-400 font-bold uppercase tracking-[0.2em] text-[10px] print:text-amber-600">
                 <div className="w-10 h-0.5 bg-amber-500/30 print:bg-amber-600" /> Complete Requirement Checklist
              </div>
            </div>
            <button
              onClick={() => window.print()}
              className="bg-white/10 hover:bg-white/20 text-white px-8 py-4 rounded-2xl font-bold text-sm backdrop-blur-md transition-all flex items-center gap-3 border border-white/20 print:hidden shadow-xl"
            >
              <FileText size={20} /> Download PDF / Print
            </button>
          </div>
        </div>
        ...
        <div className="p-8 md:p-16 space-y-16 print:p-8 print:space-y-8">
            ...
            {loan.id === 'lap' ? (
              ...
              <ul className="grid sm:grid-cols-2 gap-4 print:gap-2">
                {defaultDocs.map((doc, i) => (
                  <DocItem key={i} text={doc} index={i + 1} />
                ))}
              </ul>
            )}
          </div>
          ...
          <div className="pt-16 border-t border-slate-100 print:pt-8 print:break-inside-avoid">
            <div className="flex items-center gap-4 mb-10 print:mb-6">
              ...
              <h3 className="text-2xl font-bold text-[#0B1F3A]">Eligibility Criteria</h3>
            </div>
            ...
            <div className="mt-16 p-10 rounded-[3rem] bg-[#0B1F3A] text-white relative overflow-hidden group print:hidden">
              <div className="absolute top-0 right-0 w-64 h-64 bg-amber-500/10 rounded-full translate-x-1/3 -translate-y-1/3 blur-3xl" />
              <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-10">
                <div className="text-center md:text-left">
                  <h4 className="text-2xl font-bold mb-3">Expert Document Verification</h4>
                  <p className="text-slate-300 text-sm max-w-md">Don't let missing paperwork delay your approval. Our Surat team pre-checks your file to ensure a 98% sanction success rate.</p>
                </div>
                <button
                   onClick={() => navTo('apply')}
                   className="px-10 py-4 bg-amber-500 text-[#0B1F3A] rounded-2xl font-black shadow-2xl hover:bg-amber-400 transition-all flex items-center gap-3 whitespace-nowrap active:scale-95"
                >
                  Apply with Confidence <ArrowRight size={20} />
                </button>
              </div>
            </div>
          </div>
        ...
        {/* Footer */}
        <div className="border-t border-slate-100 p-10 text-center bg-slate-50/50 print:bg-white print:border-t-2 print:border-slate-200 print:p-6">
          <p className="text-xs text-slate-400 font-bold leading-relaxed mb-6 print:text-slate-500 print:mb-4">
            Disclaimer: This is an indicative checklist. Final document requirements may vary based on the specific lending partner's internal policies and your individual financial profile assessment.
          </p>
          <div className="flex flex-col md:flex-row justify-center items-center gap-8 text-[11px] font-black text-slate-500 uppercase tracking-widest print:text-slate-900 print:gap-4">
            <div className="flex items-center gap-2"><MapPin size={14} className="text-amber-500" /> Surat, Gujarat</div>
            <div className="hidden md:block w-1.5 h-1.5 rounded-full bg-slate-200 print:hidden" />
            <div className="flex items-center gap-2"><Phone size={14} className="text-amber-500" /> +91 99790 43073</div>
            <div className="hidden md:block w-1.5 h-1.5 rounded-full bg-slate-200 print:hidden" />
            <div className="flex items-center gap-2"><Mail size={14} className="text-amber-500" /> Mandani Associate</div>
          </div>
        </div>
      </div>

      <div className="max-w-4xl mx-auto mt-10 flex justify-between items-center text-slate-400 px-4 print:hidden">
        <button onClick={() => navTo('documents')} className="flex items-center gap-3 hover:text-[#0B1F3A] transition-all font-black text-xs uppercase tracking-widest group">
          <div className="w-8 h-8 rounded-full bg-white border border-slate-200 flex items-center justify-center group-hover:bg-[#0B1F3A] group-hover:text-white transition-colors">
            <ArrowLeft size={14} />
          </div>
          Back to all documents
        </button>
        <p className="text-[10px] font-bold uppercase tracking-widest opacity-50">Document ID: MA-REF-{loan.id.toUpperCase()}</p>
      </div>
    </div>
  );
}
...
function DocItem({ text, index }) {
  return (
    <li className="flex items-start gap-4 p-4 rounded-2xl bg-slate-50 border border-slate-100 group hover:bg-white hover:border-amber-200 transition-all print:bg-white print:p-1 print:border-none print:rounded-none">
      ...
      <span className="text-slate-700 text-sm font-bold leading-snug print:text-slate-900 print:font-medium">{text}</span>
    </li>
  );
}
